...
from tibet_iot.discovery import NetworkDiscovery
...
class TestNetworkDiscovery:
    ...
    async def test_ignore_own_beacon(self) -> None:
        """Discovery should ignore beacons from own DID."""
    ...
    async def test_handle_foreign_beacon(self) -> None:
        """Discovery should handle beacons from other devices."""
        transport = MockTransport()
        ...
        # Simulate receiving a foreign beacon
    ...
    async def test_multiple_callbacks(self) -> None:
        """Multiple on_discovered callbacks should all fire."""
        transport = MockTransport()
        ping_node = PingNode("jis:test:hub")
        discovery = NetworkDiscovery(
            device_did="jis:test:hub",
